...
const byNumber=n=>PAGES.find(p=>p.pageNumber===n)||PAGES[0];const pageIndex=n=>Math.max(0,PAGES.findIndex(p=>p.pageNumber===n));
...
function sync(){const p=byNumber(current),i=pageIndex(current);document.getElementById('pageTitle').textContent=p?.title||PROJECT.title;document.getElementById('pageNo').textContent=String(i+1);document.getElementById('prev').disabled=i<=0;document.getElementById('next').disabled=i>=PAGES.length-1}flip.on('flip',e=>{current=physical[e.data]?.source||current;sync()});
function go(delta){const i=pageIndex(current),next=PAGES[Math.max(0,Math.min(PAGES.length-1,i+delta))];if(!next||next.pageNumber===current)return;delta>0?flip.flipNext('top'):flip.flipPrev('top')}
