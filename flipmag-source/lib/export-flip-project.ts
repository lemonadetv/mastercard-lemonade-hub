...
<nav class="side-controls" aria-label="Page navigation"><button class="arrow" id="prev" aria-label="Previous page">‹</button><button class="arrow" id="next" aria-label="Next page">›</button></nav><main class="stage"><div class="viewport" id="viewport"><div class="zoom" id="zoomWrap"><div class="book" id="book"></div></div></div></main>
...
const book=document.getElementById('book'),viewport=document.getElementById('viewport'),zoom=document.getElementById('zoom'),zoomWrap=document.getElementById('zoomWrap');let current=PAGES[0]?.pageNumber||1,pan=null,turnSound=true,soundPlayed=false,soundVariant=0,navLocked=false;
...
function sync(){const p=byNumber(current),i=pageIndex(current);document.getElementById('pageTitle').textContent=p?.title||PROJECT.title;document.getElementById('pageNo').textContent=String(i+1);document.getElementById('prev').disabled=navLocked||i<=0;document.getElementById('next').disabled=navLocked||i>=PAGES.length-1;book.classList.toggle('is-front-cover',i===0);book.classList.toggle('is-back-cover',i===PAGES.length-1);document.querySelectorAll('.page-card').forEach((card,index)=>card.classList.toggle('active',index===i))}flip.on('flip',e=>{current=physical[e.data]?.source||current;sync()});flip.on('changeState',e=>{navLocked=e.data!=='read';if(e.data==='flipping'&&!soundPlayed){soundPlayed=true;playTurnSound()}if(e.data==='read')soundPlayed=false;sync()});
function go(delta){const i=pageIndex(current),next=PAGES[Math.max(0,Math.min(PAGES.length-1,i+delta))];if(navLocked||!next||next.pageNumber===current)return;navLocked=true;sync();if(delta>0)flip.flipNext('top');else flip.flipPrev('top')}
