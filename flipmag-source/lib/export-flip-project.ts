...
const book=document.getElementById('book'),viewport=document.getElementById('viewport'),zoom=document.getElementById('zoom'),zoomWrap=document.getElementById('zoomWrap');let current=PAGES[0]?.pageNumber||1,pan=null,turnSound=true,soundPlayed=false,soundVariant=0,navigationFallback=0;
...
function go(delta){const i=pageIndex(current),next=PAGES[Math.max(0,Math.min(PAGES.length-1,i+delta))];if(!next||next.pageNumber===current)return;const target=physicalIndex(next.pageNumber);flip.flip(target,'top');clearTimeout(navigationFallback);navigationFallback=setTimeout(()=>{if(current!==next.pageNumber){flip.turnToPage(target);current=next.pageNumber;sync()}},1550)}
