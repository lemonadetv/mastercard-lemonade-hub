...
const book=document.getElementById('book'),viewport=document.getElementById('viewport'),zoom=document.getElementById('zoom'),zoomWrap=document.getElementById('zoomWrap');let current=PAGES[0]?.pageNumber||1,pan=null,turnSound=true,soundPlayed=false;
...
function playTurnSound(){if(!turnSound||!window.AudioContext)return;const context=new AudioContext(),duration=.48,buffer=context.createBuffer(1,Math.ceil(context.sampleRate*duration),context.sampleRate),channel=buffer.getChannelData(0);for(let i=0;i<channel.length;i++){const p=i/channel.length;channel[i]=(Math.random()*2-1)*Math.sin(Math.PI*p)*(1-p*.45)*.34}const source=context.createBufferSource(),filter=context.createBiquadFilter(),gain=context.createGain();filter.type='bandpass';filter.frequency.setValueAtTime(1150,context.currentTime);filter.frequency.exponentialRampToValueAtTime(420,context.currentTime+duration);filter.Q.value=.72;gain.gain.setValueAtTime(.0001,context.currentTime);gain.gain.exponentialRampToValueAtTime(.22,context.currentTime+.035);gain.gain.exponentialRampToValueAtTime(.0001,context.currentTime+duration);source.buffer=buffer;source.connect(filter).connect(gain).connect(context.destination);source.start();source.onended=()=>context.close()}
